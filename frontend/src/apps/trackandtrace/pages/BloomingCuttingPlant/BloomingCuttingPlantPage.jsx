...
export default function BloomingCuttingPlantPage() {
  ...
  const loadAllCounts = async () => {
    try {
      ...
      const activeRes = await api.get('/trackandtrace/bloomingcuttingbatches/counts/?type=active');
      ...
      const destroyedRes = await api.get('/trackandtrace/bloomingcuttingbatches/counts/?type=destroyed');
    ...
    { 
      label: (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Typography component="span" sx={{ fontWeight: 'bold', fontSize: '0.75rem' }}>VERNICHTETE PFLANZEN</Typography>
  ...
  return (
    <Container maxWidth="xl" sx={{ width: '100%' }}>
      ...
      <Fade in={true} timeout={800}>
        <Box>
          <PageHeader 
            title="Blühpflanzen aus Stecklingen"

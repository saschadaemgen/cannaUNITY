...
export default function FloweringPlantPage() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Möchtest du diesen Blühpflanzen-Batch wirklich löschen?')) {
      try {
        await api.delete(`/trackandtrace/floweringbatches/${id}/`)
            ...
            >
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Box display="flex" justifyContent="space-between" width="100%">
                  <Typography>
                    <strong>{batch.seed_strain}</strong> - {batch.quantity} Blühpflanzen
